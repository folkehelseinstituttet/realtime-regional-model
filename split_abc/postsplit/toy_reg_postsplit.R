...
result_path <- paste0("/cluster/projects/nn9755k/alfonso/toy_split_october/postsplit/complete_sep",sep_county,"_sync/")



data_path <- "/cluster/projects/nn9755k/alfonso/toy_split_october/postsplit/"
...
run_calibration = T
make_main_res = F
R_calib = 19# Number of last calibration file, for running main results
...
eps1_ov = 6500 * 2/(20 * 11)*factor_ov
eps1_rest = 6500 * 9/(20 * 11)*factor_rest
#eps2=16000000
eps2_ov = 1650000*2/(20 * 11)*factor_ov
eps2_rest = 16500*9/(20 * 11)*factor_rest
N_acc = 500
num_acc = 0
...
calibration.date<-as.Date("2020-07-25")  # <-SET
start_date<-calibration.date-15
predict_from<-as.Date("2020-10-01") #Enddate    # <-SET
...
previous.calibration_file<-"./previous/R17.txt"    # <-SET
...
hosp_incidence <- hosp_incidence[date >=calibration.date]

hosp_incidence<-hosp_incidence[date<=predict_from]

hosp_incidence <- hosp_incidence[data.table(date=seq(calibration.date, predict_from, by=1)), on=c("date"="date")]
hosp_incidence<-hosp_incidence[!is.na(county_code), ]
hosp_incidence <- hosp_incidence[, N_prev:=NULL]

# Compute moving average
#hosp_incidence=hosp_incidence[, .(date, N = frollmean(N, n=pmin(1:(.N),7), align='right', adaptive=TRUE)),
#               by='county_code']
...
#for_imputation = test_data[test_data$date >= max(as.Date("2020-05-01"),calibration.date) - 6]
...
seeding <- get_seeding(first_seeding_date=start_date)
...
run_model_fylke <- function(params, rn, longrun = FALSE){ 
  ...
  idx <- params[(3 + length(reff_change_dates_nat) + length(reff_change_dates_sep)) ]
  ...
  start_se1e2iiar_pop[ , I  := initial.states[[idx]][["b_I"]] ]
  start_se1e2iiar_pop[ , Ia := initial.states[[idx]][["b_Ia"]]]
  start_se1e2iiar_pop[ , E1 := initial.states[[idx]][["b_E1"]] ]
  start_se1e2iiar_pop[ , E2 := initial.states[[idx]][["b_E2"]]  ]
  start_se1e2iiar_pop[ , R := initial.states[[idx]][["b_R"]]  ]
  start_se1e2iiar_pop[ , S  := (S - (initial.states[[idx]][["b_I"]] + 
                                       initial.states[[idx]][["b_Ia"]]+
                                       initial.states[[idx]][["b_E1"]]+
                                       initial.states[[idx]][["b_E2"]]+
                                       initial.states[[idx]][["b_R"]] ))]
...
log_lik_fylke <- function(params, rn){
  ...
  for(hosp_res in hosp_results){
    err2_ov <- 0
    err2_rest <- 0 
    hosp_res<-hosp_res[date>=calibration.date,]
    ...
    for (county in unique(reg$county_code)){
      ...
      ind1 = which(reg_c$date == max(as.Date("2020-05-01"),calibration.date))
      hosp_sub_c = hosp_sub[county_code == county, ]
      test_data_c = test_data[(test_data$county_code == county), ]
      ...
      #observed_ma = observed_ma[((calibration.date-start_date)-6):length(observed_ma)]
      ...
      #simulated_ma=simulated_ma[((calibration.date-start_date)-6):length(simulated_ma)]
      if (length(simulated_ma)!=length(observed_ma)){
        print("WARNING: DIFFERENT LENGTHS")
        print(paste0("sim: ",length(simulated_ma)))
        print(paste0("obs: ",length(observed_ma)))
      }
    ...
    err2_ov = err2_ov/count_o
    err2_rest = err2_rest/count_r
...
if(run_calibration){
  ...
  decrease.factor=0.8
  N_runs = 1
  priCov = matrix(0, nrow = length(reff_change_dates_nat) + length(reff_change_dates_sep), ncol = length(reff_change_dates_nat) + length(reff_change_dates_sep))
  diag(priCov) = rep(0.25, length(reff_change_dates_nat) + length(reff_change_dates_sep))
  
  priMean <- rep(1, length(reff_change_dates_nat) + length(reff_change_dates_sep))
  ...
  while (r < R_tot){
    ...
    if (r == 0){
      if(id == 0){
        ...
        for (j in 1:N_acc){
          ...
          PMs = c(PMs, results[2 +length(reff_change_dates_sep) + length(reff_change_dates_nat) + 1, j])
          errors1_ov = c(errors1_ov, results[2 +length(reff_change_dates_sep) + length(reff_change_dates_nat) + 2, j])
          errors1_rest = c(errors1_rest, results[2 + length(reff_change_dates_sep) + length(reff_change_dates_nat) + 3, j])
          errors2_ov = c(errors2_ov, results[2 + length(reff_change_dates_sep) + length(reff_change_dates_nat) + 4, j])
          errors2_rest = c(errors2_rest, results[2 + length(reff_change_dates_sep) + length(reff_change_dates_nat) + 5, j])
          seeds = c(seeds, results[2 + length(reff_change_dates_sep) + length(reff_change_dates_nat) + 6, j])
        }
        ...
        mpi.bcast.Robj(PM_prev, rank = 0, comm = 0)
        ...
        while(stopInd < 1){
          ...
          while((error1_ov > eps1_ov | error2_ov > eps2_ov | error1_rest > eps1_rest | error2_rest > eps2_rest ) & stopInd != 1){
            idx = sample(1:nrow(previousR), 1)
            params=rmvnorm(1, mean = priMean, sigma = priCov)
            params= c(as.numeric(previousR[idx,c("Reffs.5","Reffs.2")]),params)
        ...
        PM_prev = mpi.bcast.Robj(obj = NULL, rank = 0, comm = 0)
      ...
      if(id == 0){
        ...
        for (i in 1:N_acc){
          denom = 0
          for (j in 1:N_acc){
            denom = denom + W[j] * dmvnorm(results[3:(2+(length(reff_change_dates_sep) + length(reff_change_dates_nat))),i], mean =  Reff_prev[j, ], sigma = covMat)
          }
          if(denom == 0){
            denom = 0
          }
          W_n[i] = prior_all(results[3:(2+length(reff_change_dates_sep) + length(reff_change_dates_nat) ),i])/denom
        }
 	...
 	W[is.na(W)] = min(W, na.rm=T)
   	...
   	W[is.infinite(W)] = max(W[is.finite(W)], na.rm=T)
        ...
        # kjør simulering
        prevProp = rep(0, nrow(previousR))
        for (i in 1:nrow(previousR)){
         prevProp[i] = sum(PM_prev==i)
        }
        prevProp = prevProp + 1 # Add prior

        #prevProp = rep(1.0/(nrow(previousR)), nrow(previousR))
        ...
        while(stopInd < 1){
          ...
          while((error1_ov > eps1_ov | error1_rest > eps1_rest | error2_ov > eps2_ov | error2_rest > eps2_rest) & stopInd != 1){
            idx = sample(1:nrow(previousR), 1, prob = prevProp)
            index <- sample(1:N_acc, 1, prob = W)
            meanParams <- Reff_prev[index, ]
            ##
            params=rmvnorm(1, mean = meanParams, sigma = covMat)
            params= c(as.numeric(previousR[idx,c("Reffs.5","Reffs.2")]),params)
            
            while(any(params[3:(2 + length(reff_change_dates_sep) + length(reff_change_dates_nat))] < 0)) {
              params=rmvnorm(1, mean = meanParams, sigma = covMat)
              params= c(as.numeric(previousR[idx,c("Reffs.5","Reffs.2")]),params)
            }
    ...
    number_complete = 0
    if(id == 0){
      resLast <- data.frame("R0" = R0s, "Reffs" = Reffs, "PMs" = PMs,"seeds" = seeds, "error1s_ov" = errors1_ov, "error2s_ov" = errors2_ov, "errors1_rest" = errors1_rest, "errors2_rest" = errors2_rest)
